#pragma once
#include <algorithm>
namespace PerlinNoiseFixed{

		static inline int lerp(int t, int a, int b) { return a + (t * (b - a) >> 12); }
		static inline int grad(int hash, int x, int y, int z) {
			int h = hash & 15, u = h<8 ? x : y, v = h<4 ? y : h == 12 || h == 14 ? x : z;
			return ((h & 1) == 0 ? u : -u) + ((h & 2) == 0 ? v : -v);
		}

		static inline double f(double t) { return t * t * t * (t * (t * 6 - 15) + 10); }
		...
		static inline int fade(int t) {
			...
			int t0 = fade[t >> 8], t1 = fade[std::min(255, (t >> 8) + 1)];
			return t0 + ((t & 255) * (t1 - t0) >> 8);
		}

		static inline int noise(int x, int y, int z) {
